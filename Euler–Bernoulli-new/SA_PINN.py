...
class SA_PINN:
    ...
    def __init__(self,mat_filename,layers:[],tf_iter:int,newton_iter:int,f_model,ux_model=u_x_model,Loss=__DefaultLoss,lbfgs_lr=0.8,N_f=10000, id_t=0, u0_t= 0, v0_t= 0,checkPointPath="./checkPoint"):
        self.N_f=N_f
        self.u0_t= u0_t
        self.id_t= id_t
        ...
        self.N_f=N_f
        self.col_weights = tf.Variable(tf.reshape(tf.repeat(100.0, self.N_f),(self.N_f, -1)))
        self.u_weights=tf.Variable(tf.reshape(tf.repeat(100.0, self.u0.shape[0]),(self.u0.shape[0], -1)))
        self.ut_weights=tf.Variable(tf.reshape(tf.repeat(100.0, self.u0.shape[0]),(self.u0.shape[0], -1)))
        self.u_lb_weights=tf.Variable(tf.reshape(tf.repeat(100.0, self.u_lb.shape[0]),(self.u_lb.shape[0], -1)))
        self.u_ub_weights=tf.Variable(tf.reshape(tf.repeat(100.0, self.u_ub.shape[0]),(self.u_ub.shape[0], -1)))
        self.u_lb_xx_weights=tf.Variable(tf.reshape(tf.repeat(100.0, self.u_lb.shape[0]),(self.u_lb.shape[0], -1)))
        self.u_ub_xx_weights=tf.Variable(tf.reshape(tf.repeat(100.0, self.u_ub.shape[0]),(self.u_ub.shape[0], -1)))
    ...
    def __Loadmat(self,fileName):
        ...
        Exact = data['Exact']#.T
        Exact_ut = data['Exact_u_t']#.T
        Exact_uxx = data['Exact_u_x2']#.T
#        Exact_vt = data['Exact_v_t']#.T
        print('Exact.shape= ', Exact.shape)

#        self.Exact=Exact
        self.Exact_u = np.real(Exact).T
#        self.Exact_v = Exact.imag.T
        self.Exact_u_t = Exact_ut.T
        self.Exact_u_xx = Exact_uxx.T
...
#        self.u_star = Exact.flatten()[:, None] 
        ...
        ut_star_ori= self.Exact_u_t[:, (self.id_t*tint):((self.id_t +1)*tint + 1)]
        ubxx_star_ori= self.Exact_u_xx[:, (self.id_t*tint):((self.id_t +1)*tint + 1)]
        print('u_star_ori.shape= ', u_star_ori.shape)

        self.u_star=u_star_ori#.flatten()[:,None]
        self.ut_star=ut_star_ori#.flatten()[:,None]
        ...
        self.u0td = tf.cast(self.Exact_u_t[idx_x,0:1], dtype = tfdoubstr)
        self.u_lbxxtd = tf.cast(self.Exact_u_xx[0:1, idx_t], dtype = tfdoubstr)
        self.u_ubxxtd = tf.cast(self.Exact_u_xx[-1, idx_t], dtype = tfdoubstr)
...
#            print('u0_t.shape= ', u0_t.shape)
    ...
    @tf.function
    def __grad(self, x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch,u0td_batch,u_lbxxtd_batch, u_ubxxtd_batch,u_lb_batch,u_ub_batch,x_lb, t_lb, x_ub, t_ub,col_weights,u_weights,ut_weights,u_lb_weights,u_ub_weights, u_lb_xx_weights, u_ub_xx_weights):
        with tf.GradientTape(persistent=True) as tape:
            loss_value, mse_0, mse_f = self.Loss(self,x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch,u0td_batch,u_lbxxtd_batch, u_ubxxtd_batch,u_lb_batch,u_ub_batch, x_lb, t_lb, x_ub, t_ub,col_weights,u_weights,ut_weights,u_lb_weights,u_ub_weights, u_lb_xx_weights, u_ub_xx_weights)
            grads = tape.gradient(loss_value, self.model.trainable_variables)
            #print(grads)
            grads_col = tape.gradient(loss_value, col_weights)
            grads_u = tape.gradient(loss_value, u_weights)
            grads_ut = tape.gradient(loss_value, ut_weights)
            grads_u_lb = tape.gradient(loss_value, u_lb_weights)
            grads_u_ub = tape.gradient(loss_value, u_ub_weights)
            grads_u_lb_xx = tape.gradient(loss_value, u_lb_xx_weights)
            grads_u_ub_xx = tape.gradient(loss_value, u_ub_xx_weights)

        return loss_value, mse_0, mse_f, grads, grads_col, grads_u, grads_ut, grads_u_lb, grads_u_ub, grads_u_lb_xx, grads_u_ub_xx#, SA_grads
    ...
    def fit(self):
        ...
        tf_optimizer = tf.keras.optimizers.Adam(lr = 0.005, beta_1=.99, beta_2=0.999,epsilon=1e-8)
#        tf_optimizer_coll = tf.keras.optimizers.Adam(lr = 0.005, beta_1=.90)
#        tf_optimizer_u = tf.keras.optimizers.Adam(lr = 0.005, beta_1=.90)
        tf_optimizer_weights = tf.keras.optimizers.Adam(lr = 0.005, beta_1=.99, beta_2=0.999,epsilon=1e-8)
        ...
        for epoch in range(self.tf_iter):
            for i in range(n_batches):
                ...
                u0td_batch = self.u0td#[i*batch_sz:(i*batch_sz + batch_sz),]
                u_lbxxtd_batch = self.u_lbxxtd#[i*batch_sz:(i*batch_sz + batch_sz),]
                u_ubxxtd_batch = self.u_ubxxtd#[i*batch_sz:(i*batch_sz + batch_sz),]
                ...
                loss_value,mse_0, mse_f, grads, grads_col, grads_u, grads_ut, grads_u_lb, grads_u_ub, grads_u_lb_xx, grads_u_ub_xx = self.__grad(x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch,u0td_batch,u_lbxxtd_batch, u_ubxxtd_batch,u_lb_batch,u_ub_batch,self.x_lb, self.t_lb, self.x_ub, self.t_ub,self.col_weights,self.u_weights,self.ut_weights,self.u_lb_weights,self.u_ub_weights, self.u_lb_xx_weights, self.u_lb_xx_weights)

                tf_optimizer.apply_gradients(zip(grads, self.model.trainable_variables))
                tf_optimizer_weights.apply_gradients(zip([-grads_col, -grads_u, -grads_ut, -grads_u_lb, -grads_u_ub, -grads_u_lb_xx, -grads_u_ub_xx], [self.col_weights, self.u_weights,self.ut_weights,self.u_lb_weights,self.u_ub_weights, self.u_lb_xx_weights, self.u_lb_xx_weights]))

            if (epoch+1) % 100 == 0:
                error_u_value = self.error_u()

                elapsed = time.time() - start_time
                print('It: %d, Time: %.2f, mse_0: %.4e, mse_f: %.4e, total loss: %.4e, error u: %.4e' % (epoch+1, elapsed, mse_0, mse_f, loss_value, error_u_value))
                start_time = time.time()
        ...
        loss_and_flat_grad = self.__get_loss_and_flat_grad(x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch,u0td_batch,u_lbxxtd_batch, u_ubxxtd_batch,u_lb_batch,u_ub_batch, self.x_lb, self.t_lb, self.x_ub, self.t_ub, self.col_weights, self.u_weights)
...
# L-BFGS implementation from https://github.com/pierremtb/PINNs-TF2.0
    def __get_loss_and_flat_grad(self,x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch,u0td_batch,u_lbxxtd_batch, u_ubxxtd_batch,u_lb_batch,u_ub_batch, x_lb, t_lb, x_ub, t_ub, col_weights, u_weights):
        def loss_and_flat_grad(w):
            with tf.GradientTape() as tape:
                self.__set_weights(self.model, w, self.sizes_w, self.sizes_b)
                loss_value, _, _ = self.Loss(self,x_f_batch, t_f_batch, x0_batch, t0_batch, u0_batch,u0td_batch,u_lbxxtd_batch, u_ubxxtd_batch,u_lb_batch,u_ub_batch, x_lb, t_lb, x_ub, t_ub,self.col_weights,self.u_weights,self.ut_weights,self.u_lb_weights,self.u_ub_weights,self.u_lb_xx_weights,self.u_ub_xx_weights)
    ...
    def error_u(self):
        ...
        u_pred,_,_ = self.uv_model(X_star[:, 0:1], X_star[:, 1:2])
        u_star = self.Exact_u.T[(self.id_t*tint):((self.id_t +1)*tint + 1),:]
        u_star = u_star.flatten()[:, None]
#        u_star = self.Exact_u##
#        u_star = tf.reshape(u_star, [-1, 1])
        u_pred= u_pred.numpy()

        error_u = np.linalg.norm(u_star-u_pred,2)/np.linalg.norm(u_star,2)
#        u_star = tf.cast(u_star, tf.float64)
#        error_u = tf.norm(u_star - u_pred, ord=2) / tf.norm(u_star, ord=2)

        return error_u

    def predict(self):
        X_star = tf.convert_to_tensor(self.X_star, dtype=tfdoubstr)
        u_star,_,_ = self.uv_model(X_star[:,0:1],
                        X_star[:,1:2])
