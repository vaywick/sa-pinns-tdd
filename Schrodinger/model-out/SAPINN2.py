...
class SA_PINN2(SA_PINN):
    ...
    def __init__(self, mat_filename, layers: [], adam_iter: int, newton_iter: int, f_model, Loss=..., lbfgs_lr=0.8, N_f=10000, id_t=0, u0_t=0,v0_t=0, checkPointPath="./checkPoint"):
        super().__init__(mat_filename, layers, adam_iter, newton_iter, f_model, Loss, lbfgs_lr, N_f, id_t, u0_t, checkPointPath)
        self.v0_t=v0_t
        self.__Loadmat(mat_filename)
        self.SA_weights['col_v_weights']=nn.Parameter(torch.full((N_f, 1), 100.0, device=device))
        self.SA_weights['v_weights']=nn.Parameter(torch.full((self.x0.shape[0], 1), 100.0, device=device))
        self.SA_weights['v_lb_weights']=nn.Parameter(torch.full((self.x_lb.shape[0], 1), 100.0, device=device))
        self.SA_weights['v_ub_weights']=nn.Parameter(torch.full((self.x_ub.shape[0], 1), 100.0, device=device))
    ...
    def __Loadmat(self, fileName):
        ...
        self.lables_0=(self.u0,self.v0)
        self.lables_lb=(self.u_lb,self.v_lb)
        self.lables_ub=(self.u_ub,self.v_ub)
...
#        u_pred,v_pred, f_u_pred, f_v_pred = self.predict()#self.u_model, self.X_star)
    ...
    def predict(self):
        X_star = torch.tensor(self.X_star, dtype=torch.float32, device=device, requires_grad=True)
        u_star,v_star,_, _ = self.uv_model(X_star[:, 0:1], X_star[:, 1:2])
