...
def loss(sapinn:SA_PINN2, x_f_batch, t_f_batch, x0, t0, lables_0, lables_lb, lables_ub, x_lb, t_lb, x_ub, t_ub, SA_weights):
    ...
    u_ub,v_ub=lables_ub
    u_lb,v_lb=lables_lb
    mse_0_u = torch.mean((SA_weights['u_weights'] * (u0 - u0_pred))**2)+torch.mean((SA_weights['v_weights'] * (v0 - v0_pred))**2)
    mse_b_u = torch.mean(( SA_weights['u_lb_weights']*(u_lb_pred - u_lb))**2) +\
          torch.mean(( SA_weights['u_ub_weights']*(u_ub_pred - u_ub))**2)+\
          torch.mean(( SA_weights['v_ub_weights']*(v_ub_pred - v_ub))**2)+\
          torch.mean(( SA_weights['v_lb_weights']*(v_lb_pred - v_lb))**2)
          
    mse_f_u = torch.mean((SA_weights['col_weights'] * f_u_pred)**2)+torch.mean((SA_weights['col_v_weights'] * f_v_pred)**2)
...
def f_model(sapinn:SA_PINN2, x, t):
...
for id_t in range(Nm):
    model=SA_PINN2("data-sch.mat",layer_sizes,tf_iter1[id_t],newton_iter1[id_t],f_model=f_model,Loss=loss,N_f= n_f, id_t= id_t, u0_t= u0_t,v0_t=v0_t)

    model.fit()
    model.fit_lbfgs()
    
    # Save the model
    torch.save(model.model.state_dict(), model.checkPointPath + "/final.pth")
