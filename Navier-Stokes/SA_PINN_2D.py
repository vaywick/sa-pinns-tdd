...
class SA_PINN_2D:
    ...
    def __init__(self, mat_filename, layers, tf_iter, newton_iter, f_model, ux_model=None, Loss=None, lbfgs_lr=0.8, N_f=10000, id_t=0, u0_t= u0_t, v0_t= v0_t, p0_t= p0_t, checkPointPath="./checkPoint"):
        self.N_f = N_f
        self.id_t = id_t
        self.u0_t= u0_t
        self.v0_t= v0_t
        ...
        self.col_weights = torch.full((N_f, 1), 1.0, device=device, requires_grad=True)
        self.col_v_weights = torch.full((N_f, 1), 1.0, device=device, requires_grad=True)
        self.u_weights = torch.rand((self.u0.shape[0], 1), device=device, requires_grad=True)
        self.ub_weights = torch.rand((self.u_x_lb.shape[0], 1), device=device, requires_grad=True)
        ...
        self.p_weights = torch.rand((self.p0.shape[0], 1), device=device, requires_grad=True)
        self.pb_weights = torch.rand((self.p_x_lb.shape[0], 1), device=device, requires_grad=True)
    ...
    def uvp_model(self, x, y, t):
        ...
        uvp = self.u_model(torch.cat([x, y, t], dim=1))
        u = uvp[:,0:1]
        v = uvp[:,1:2]
        p = uvp[:,2:3]
        return u, v, p

    def g_grad(self, model, x_f, y_f, t_f, t0, u0, v0, p0, x_lb, x_ub, y_lb, y_ub, u_x_lb, u_x_ub, u_y_lb, u_y_ub, v_x_lb, v_x_ub, v_y_lb, v_y_ub, p_x_lb, p_x_ub, p_y_lb, p_y_ub, XY, XT, YT, col_weights, u_weights, ub_weights, col_v_weights, v_weights, vb_weights, p_weights, pb_weights):
        self.u_model.train()
        ...
        p0 = p0.requires_grad_(True).float()
        ...
        p_x_lb = p_x_lb.requires_grad_(True).float()
        p_x_ub = p_x_ub.requires_grad_(True).float()
        p_y_lb = p_y_lb.requires_grad_(True).float()
        p_y_ub = p_y_ub.requires_grad_(True).float()
        ...
        # Forward pass
        loss_value, mse_0, mse_b, mse_f = self.Loss(model, x_f, y_f, t_f, t0, u0, v0, p0, x_lb, x_ub, y_lb, y_ub, u_x_lb, u_x_ub, u_y_lb, u_y_ub, v_x_lb, v_x_ub, v_y_lb, v_y_ub, p_x_lb, p_x_ub, p_y_lb, p_y_ub, XY, XT, YT, col_weights, u_weights, ub_weights, col_v_weights, v_weights, vb_weights, p_weights, pb_weights)
        ...
        grads_p = self.p_weights.grad.clone()
        grads_pb = self.pb_weights.grad.clone()

        return loss_value.item(), mse_0.item(), mse_b.item(), mse_f.item(), grads, grads_col, grads_u, grads_ub, grads_col_v, grads_v, grads_vb, grads_p, grads_pb
    ...
    def fit(self):
        ...
        optimizer_p_weights = optim.Adam([self.p_weights], lr=0.005, betas=(0.99, 0.999))
        optimizer_pb_weights = optim.Adam([self.pb_weights], lr=0.005, betas=(0.99, 0.999))
    
        print("starting Adam training")

        for epoch in range(self.tf_iter):
            for i in range(n_batches):
                loss_value, mse_0, mse_b, mse_f, grads, grads_col, grads_u, grads_ub, grads_col_v, grads_v, grads_vb, grads_p, grads_pb= self.g_grad(self.u_model, self.x_f, self.y_f, self.t_f, self.t0, self.u0, self.v0, self.p0, self.x_lb, self.x_ub, self.y_lb, self.y_ub, self.u_x_lb, self.u_x_ub, self.u_y_lb, self.u_y_ub, self.v_x_lb, self.v_x_ub, self.v_y_lb, self.v_y_ub, self.p_x_lb, self.p_x_ub, self.p_y_lb, self.p_y_ub, self.XY, self.XT, self.YT, self.col_weights, self.u_weights, self.ub_weights, self.col_v_weights, self.v_weights, self.vb_weights, self.p_weights, self.pb_weights)
                ...
                optimizer_p_weights.zero_grad()
                self.p_weights.grad = -grads_p
                optimizer_p_weights.step()
    
                optimizer_pb_weights.zero_grad()
                self.pb_weights.grad = -grads_pb
                optimizer_pb_weights.step()
    
            if (epoch+1) % 100 == 0:
                elapsed = time.time() - start_time
                error_u_value, error_v_value, error_p_value = self.error_u()
                print('It: %d, Time: %.2f, mse_0: %.4e, mse_f: %.4e, total loss: %.4e, Error u,v,p: %.4e %.4e %.4e' % (epoch+1, elapsed, mse_0, mse_f, loss_value, error_u_value, error_v_value, error_p_value))
                start_time = time.time()
    ...
    def fit_lbfgs(self):
        ...
        optimizer_p_weights = optim.LBFGS([self.p_weights], lr=0.8)
        optimizer_pb_weights = optim.LBFGS([self.pb_weights], lr=0.8)
...
#                    break
    ...
    def __Loadmat(self, fileName):
        ...
        p0all_ori= p_star_ori[:,:,0]
        p0all=p0all_ori.flatten()[:,None]
        p0 = p0all[selected_indices]

        p_x_lball_ori= p_star_ori[:,0,:].T
        p_x_uball_ori= p_star_ori[:,-1,:].T
        p_y_lball_ori= p_star_ori[0, :,:].T
        p_y_uball_ori= p_star_ori[-1,:,:].T

        p_x_lball =p_x_lball_ori.flatten()[:,None]
        p_x_uball =p_x_uball_ori.flatten()[:,None]
        p_y_lball =p_y_lball_ori.flatten()[:,None]
        p_y_uball =p_y_uball_ori.flatten()[:,None]
        ...
        p_x_lb = p_x_lball[selected_indicesyt]
        p_x_ub = p_x_uball[selected_indicesyt]
        p_y_lb = p_y_lball[selected_indicesxt]
        p_y_ub = p_y_uball[selected_indicesxt]
        ...
        self.p_x_lb = torch.tensor(p_x_lb, dtype=torch.float32)
        self.p_x_ub = torch.tensor(p_x_ub, dtype=torch.float32)
        self.p_y_lb = torch.tensor(p_y_lb, dtype=torch.float32)
        self.p_y_ub = torch.tensor(p_y_ub, dtype=torch.float32)
...
#        self.X_star = np.hstack((X.flatten()[:, None], Y.flatten()[:, None], T.flatten()[:, None]))
    ...
    def error_u(self):
        ...
        with torch.no_grad():
            u_pred, v_pred, p_pred = self.uvp_model(X_star[:, 0:1], X_star[:, 1:2], X_star[:, 2:3])
        ...
        p_star = torch.tensor(self.p_star, dtype=torch.float32, device=device)
    
        # Calculate the error
        error_u = torch.norm(u_star - u_pred, p=2) / torch.norm(u_star, p=2)
        error_v = torch.norm(v_star - v_pred, p=2) / torch.norm(v_star, p=2)
        error_p = torch.norm(p_star - p_pred, p=2) / torch.norm(p_star, p=2)
    
        return error_u.item(), error_v.item(), error_p.item()
    ...
    def predict(self, x,y,t):
        ...
        with torch.no_grad():  # Disable gradient computation for u_star
            u_star, v_star, p_star = self.uvp_model(X_star[:, 0:1], X_star[:, 1:2], X_star[:, 2:3])
    
        # Compute f_u_star with gradients
        f_u_star, f_v_star, div_star = self.f_model(self.u_model, X_star[:, 0:1], X_star[:, 1:2], X_star[:, 2:3])
    
        return u_star.detach().cpu().numpy(), f_u_star.detach().cpu().numpy(), v_star.detach().cpu().numpy(), f_v_star.detach().cpu().numpy(), p_star.detach().cpu().numpy(), div_star.detach().cpu().numpy()
