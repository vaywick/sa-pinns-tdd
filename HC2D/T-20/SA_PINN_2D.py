...
class SA_PINN_2D:
    ...
    def __init__(self, mat_filename, layers, tf_iter, newton_iter, f_model, ux_model=None, Loss=None, lbfgs_lr=0.8, N_f=10000, id_t=0, u0_t= u0_t, checkPointPath="./checkPoint"):
        self.N_f = N_f
        self.id_t = id_t
        self.u0_t= u0_t
        ...
        self.u_weights = torch.full((self.u0.shape[0], 1), 100., device=device, requires_grad=True)
        self.ub_weights = torch.full((self.u_x_lb.shape[0], 1), 100., device=device, requires_grad=True)
...
#        return u
    ...
    def u_x_model(self, x, y, t):
        ...
        self.lb = self.lb.to(device).float()
        self.ub = self.ub.to(device).float()
...
#        return self.u_model(torch.cat([x,y,z, t], dim=1))
        return u

#    def u_x_model(self, x, y, t):
#        # 确保 x, y, 和 t 需要梯度并位于正确的设备上
#        x = x.requires_grad_(True).to(device).float()
#        y = y.requires_grad_(True).to(device).float()
#        t = t.requires_grad_(True).to(device).float()
#    
#        # 把 NumPy 数组转换为 PyTorch 张量并移到指定设备
#        if not isinstance(self.lb, torch.Tensor):
#            self.lb = torch.tensor(self.lb, device=device, dtype=torch.float32)
#        if not isinstance(self.ub, torch.Tensor):
#            self.ub = torch.tensor(self.ub, device=device, dtype=torch.float32)
#    
#        X = torch.cat([x, y, t], dim=1)
...
#        return u
    ...
    def g_grad(self, model, x_f, y_f, t_f, t0, u0, x_lb, x_ub, y_lb, y_ub, u_x_lb, u_x_ub, u_y_lb, u_y_ub, XY, XT, YT, col_weights, u_weights, ub_weights):#SA_weights):
        self.u_model.train()
        ...
        # Forward pass
        loss_value, mse_0, mse_b, mse_f = self.Loss(self, x_f, y_f, t_f, t0, u0, x_lb, x_ub, y_lb, y_ub, u_x_lb, u_x_ub, u_y_lb, u_y_ub, XY, XT, YT, col_weights, u_weights, ub_weights)
        ...
        return loss_value.item(), mse_0.item(), mse_b.item(), mse_f.item(), grads, grads_col, grads_u, grads_ub
    ...
    def fit(self):
        ...
        for epoch in range(self.tf_iter):
            for i in range(n_batches):
                loss_value, mse_0, mse_b, mse_f, grads, grads_col, grads_u, grads_ub = self.g_grad(self.u_model, self.x_f, self.y_f, self.t_f, self.t0, self.u0, self.x_lb, self.x_ub, self.y_lb, self.y_ub, self.u_x_lb, self.u_x_ub, self.u_y_lb, self.u_y_ub, self.XY, self.XT, self.YT, self.col_weights, self.u_weights, self.ub_weights)#sself.SA_weights)
    ...
    def __Loadmat(self, fileName):
        ...
        self.u_star_all=data["Exact"]
        self.refer_u=data["Exact"] 

        u_star_ori=data["Exact"][:,:, (self.id_t*tint):((self.id_t +1)*tint + 1)]
...
#        self.x_lb = torch.tensor(lb[0, 0], dtype=torch.float32)
#        self.x_ub = torch.tensor(ub[0, 0], dtype=torch.float32)
#        self.y_lb = torch.tensor(lb[0, 1], dtype=torch.float32)
#        self.y_ub = torch.tensor(ub[0, 1], dtype=torch.float32)
#        self.t0 = torch.tensor(t[0:1, :], dtype=torch.float32)
...
#        self.u_star = data["Exact"].flatten()[:, None]
    ...
    def predict(self, x,y,t):
        ...
        f_u_pred = self.f_model(self, X_star[:, 0:1], X_star[:, 1:2], X_star[:, 2:3])
    
        return u_pred.detach().cpu().numpy(), f_u_pred.detach().cpu().numpy()
