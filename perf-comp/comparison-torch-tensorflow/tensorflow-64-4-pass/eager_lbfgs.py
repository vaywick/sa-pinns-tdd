...
final_loss = None
times = []
def lbfgs(u_model,opfunc, x, state, maxIter = 100, learningRate = 1, do_verbose = True):
...
#    start_time = time.time()
    ...
    if do_verbose:
      if (nIter+1) % 100 == 0:
        ...
        print("Step %3d time: %.2f loss %6.5f "%(nIter+1, elapsed, f.numpy()))
        start_time = time.time()

      loss_l.append(f.numpy())
