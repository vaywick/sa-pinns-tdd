...
def loss(model,x_f,y_f, t_f,u0,u_x_lb,u_x_ub,u_y_lb,u_y_ub, XY, XT, YT,SA_weight):
    ...
    u_x_lb_pred= model.u_x_model(model.x_lb, YT[:,0:1],YT[:,1:2])
    u_x_ub_pred= model.u_x_model(model.x_ub, YT[:,0:1],YT[:,1:2])
    u_y_lb_pred= model.u_x_model( XT[:,0:1],model.y_lb,XT[:,1:2])
    u_y_ub_pred= model.u_x_model( XT[:,0:1],model.y_ub,XT[:,1:2])

    mse_0_u = tf.reduce_mean(tf.square(SA_weight["u_weights"]*(u0 - u0_pred)))

    mse_b_u = tf.reduce_mean(tf.square(u_x_lb_pred - u_x_lb)) + \
            tf.reduce_mean(tf.square(u_x_ub_pred - u_x_ub)) +\
            tf.reduce_mean(tf.square(u_y_ub_pred - u_y_ub)) +\
            tf.reduce_mean(tf.square(u_y_lb_pred - u_y_lb))

    mse_f_u = tf.reduce_mean(tf.square(SA_weight["col_weights"]*f_u_pred))
    return  mse_0_u + mse_b_u + mse_f_u, mse_0_u, mse_f_u


model=SA_PINN_2D("data.mat",layer_sizes,tf_iter1,newton_iter1,f_model=f_model,Loss=loss,N_f= n_f)
model.fit()
...
xx= np.linspace(-X, X, N_x)
yy= np.linspace(-Y, Y, N_y)
tt= np.linspace(-T, T, N_t)
Ntinter = 2
