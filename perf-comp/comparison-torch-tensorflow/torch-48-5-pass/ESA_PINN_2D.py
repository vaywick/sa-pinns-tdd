...
class ESA_PINN_2D:
    ...
    def __init__(self, mat_filename, layers, tf_iter, newton_iter, f_model, ux_model=None, Loss=None, lbfgs_lr=0.8, N_f=10000, checkPointPath="./checkPoint"):
        ...
        self.u_weights = torch.full((self.u0.shape[0], 1), 1.0, device=device, requires_grad=True)
        self.ub_weights = torch.full((self.u_x_lb.shape[0], 1), 1.0, device=device, requires_grad=True)
    ...
    def g_grad(self, model, x_f, y_f, t_f, t0, u0, x_lb, x_ub, y_lb, y_ub, u_x_lb, u_x_ub, u_y_lb, u_y_ub, XY, XT, YT, col_weights, u_weights, ub_weights):
        self.u_model.train()
...
#        u_weights = u_weights.to(device).requires_grad_(True)
        ...
        loss_value, mse_0, mse_b, mse_f = self.Loss(model, x_f, y_f, t_f, t0, u0, x_lb, x_ub, y_lb, y_ub, u_x_lb, u_x_ub, u_y_lb, u_y_ub, XY, XT, YT, col_weights, u_weights, ub_weights)
    ...
    def fit(self):
        ...
        for epoch in range(self.tf_iter):
            for i in range(n_batches):
                loss_value, mse_0, mse_b, mse_f, grads, grads_col, grads_u, grads_ub = self.g_grad(self.u_model, self.x_f, self.y_f, self.t_f, self.t0, self.u0, self.x_lb, self.x_ub, self.y_lb, self.y_ub, self.u_x_lb, self.u_x_ub, self.u_y_lb, self.u_y_ub, self.XY, self.XT, self.YT, self.col_weights, self.u_weights, self.ub_weights)
    ...
    def fit_lbfgs(self):
        ...
        optimizer = optim.LBFGS(self.u_model.parameters(), lr=.5, tolerance_grad=1e-05, tolerance_change=1e-09)
#        optimizer = optim.LBFGS(self.u_model.parameters(), lr=0.8, tolerance_grad=1e-05, tolerance_change=1e-09)
        ...
        for epoch in range(self.newton_iter):
            ...
            if (epoch+1) % 100 == 0:
                ...
                a3= lossl_value[-3]
                a4= lossl_value[-4]
                if epoch>9000 and abs((a1-a2)/a2)<1e-10 and abs((a2-a3)/a3)<1e-10 and abs((a3-a4)/a4)<1e-10:
                    break
