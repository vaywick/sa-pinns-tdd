...
import os
from globalf import *
from ESA_PINN_2D import ESA_PINN_2D
...
for id_t in range(Nm):
    model = ESA_PINN_2D("data.mat", layer_sizes, tf_iter1, newton_iter1, f_model=f_model, Loss=loss, N_f=n_f)
    model.fit()
    model.fit_lbfgs()
...
# domain decomposition in prediction stage
