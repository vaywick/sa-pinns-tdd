...
class SA_PINN:
    ...
    def __init__(self,mat_filename,layers:[],adam_iter:int,newton_iter:int,f_model,Loss=__DefaultLoss,lbfgs_lr=0.8,N_f=10000, id_t=0, u0_tmp= 0,checkPointPath="./checkPoint"):
        self.N_f=N_f
        self.u0_tmp= u0_tmp
        self.id_t= id_t
...
#                    break
    ...
    def __Loadmat(self,fileName):
        ...
        self.Exact_u_t = data['Exact_u_t'].T
        self.Exact_v_t = data['Exact_v_t'].T
...
#        self.u0 = torch.tensor(self.Exact_u[idx_x, 0:1], dtype=torch.float32).cuda()
        ...
        if self.id_t==0:
            self.u0 = torch.tensor(self.Exact_u[idx_x, 0:1], dtype=torch.float32).cuda()
            self.u0_t = torch.tensor(self.Exact_u_t[idx_x, 0:1], dtype=torch.float32).cuda()
        else:
