...
def loss(sapinn:SA_PINN2, x_f_batch, t_f_batch, x0, t0, u0, v0, u0_t, v0_t, x_lb, t_lb, x_ub, t_ub, u_lb, u_ub, v_lb, v_ub, col_weights, u_weights, u_lb_weights, u_ub_weights, col_v_weights, v_weights, v_lb_weights, v_ub_weights):
    u0_pred,v0_pred,u0_t_pred,v0_t_pred = sapinn.uv_model(x0,t0)
    ...
    mse_0_u = torch.mean((u_weights * (u0 - u0_pred))**2)+ torch.mean((v_weights * (v0 - v0_pred))**2) + \
              torch.mean((u_weights * (u0_t - u0_t_pred))**2)+ torch.mean((v_weights * (v0_t - v0_t_pred))**2)
...
def f_model(sapinn:SA_PINN2, x, t):
    ...
    u, v, u_t, v_t = sapinn.uv_model(x, t)
    ...
    v_tt = autograd.grad(v_t, t, grad_outputs=torch.ones_like(v_t), create_graph=True)[0]
    ...
    f_u= u_tt - u_xx + u - v_x
    f_v= v_tt + u_x - v_xx + v - torch.cos(t)

    return f_u, f_v

for id_t in range(Nm):
    model=SA_PINN2("data-tb.mat",layer_sizes,tf_iter1[id_t],newton_iter1[id_t],f_model=f_model,Loss=loss,N_f= n_f, id_t= id_t, u0_tmp= u0_tmp,v0_tmp=v0_tmp)
